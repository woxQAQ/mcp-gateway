<template>
  <div class="h-16 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm border-b border-gray-200/50 dark:border-gray-700/50 flex-between px-6 relative z-50">
    ...
    <div class="flex items-center space-x-4">
      ...
      <div class="flex items-center space-x-3">
        <div class="w-10 h-10 bg-gradient-to-br from-blue-500 via-purple-500 to-pink-500 rounded-xl flex-center shadow-lg">
          <el-icon class="text-white" size="20"><Setting /></el-icon>
        </div>
        <div class="hidden sm:block">
          <h1 class="text-xl font-bold bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">
            MCP Gateway
          </h1>
          <p class="text-xs text-gray-500 dark:text-gray-400">管理系统</p>
        </div>
      </div>
    </div>
    ...
    <div class="flex items-center space-x-3">
      ...
      <!-- 用户菜单 -->
      <el-dropdown @command="handleUserMenuClick" trigger="click">
...
const handleUserMenuClick = (command: string) => {
  console.log('User menu clicked:', command)
  switch (command) {
    case 'profile':
      // 跳转到个人资料
      break
